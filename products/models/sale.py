...
class Sale(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.doc_no:
            ...
            last_sale = Sale.objects.filter(doc_no__startswith=f'{prefix}-{today_str}').order_by('doc_no').last()
            ...
            if last_sale:
                ...
                new_id = 1
            self.doc_no = f"SAL-{today_str}-{new_id:04d}"
...
class SaleItem(models.Model):
    ...
    def __str__(self):
        return f"{self.sale.doc_no} - {self.product.name} ({self.quantity} {self.product.unit})"
